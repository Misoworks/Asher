use crate::{
    ...
    output::{NestedOutput, OutputDescriptor, create_output},
...
use staccato_ipc::{ShellStatus, XwaylandStatus};
use staccato_layout::{LayoutEngine, LayoutError, Rect, WindowId, WindowInfo, WorkspaceId};
...
impl BatonState {
    pub fn new(display: &DisplayHandle, config: StaccatoConfig) -> Self {
    ...
    pub fn new_for_output(
        ...
        let seat = seat_state.new_wl_seat(display, "staccato-seat");
        let output = create_output(display, &output_descriptor);
        let mut layout = layout_from_config(&config);
        layout.set_bounds(Rect::new(
            0,
            0,
            output_descriptor.size.w,
            output_descriptor.size.h,
        ));
        ...
        Self {
            ...
            keyboard: None,
            output,
            output_size: output_descriptor.size,
            output_refresh_millihertz: output_descriptor.refresh_millihertz,
            layout,
    ...
    pub fn map_toplevel(&mut self, surface: ToplevelSurface) {
        ...
        let info = WindowInfo::new(staccato_layout::WindowId(0), workspace.clone(), geometry);
