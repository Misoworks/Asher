...
use staccato_layout::{ModeId, ProfileId, Rect, WindowId, WindowState, WorkspaceId};
use std::{
    env, fs, io,
...
pub const SOCKET_ENV: &str = "STACCATO_IPC_SOCKET";
pub const SHELL_SOCKET_ENV: &str = "STACCATO_SHELL_SOCKET";
...
pub fn socket_path() -> PathBuf {
    ...
    runtime_dir().join("staccato").join("baton.sock")
}
...
fn runtime_dir() -> PathBuf {
    env::var_os("XDG_RUNTIME_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| env::temp_dir().join(format!("staccato-{}", current_user())))
}
