...
declare global {
  interface Window {
    __ASHER_INITIAL_STATE__?: ShellSnapshot;
    __ASHER_SURFACE__?: ShellSurface;
    fenestra?: {
      bridge?: NativeBridge;
...
let currentSnapshot = normalizeSnapshot(window.__ASHER_INITIAL_STATE__);
...
export const sendAction = (action: ShellAction) => {
  ...
  if (bridge?.commands.includes("asher.action")) {
    void bridge.invoke(
      "asher.action",
...
async function initializeNativeBridge() {
  ...
  bridge.listen?.<ShellSnapshot>("asher.snapshot", applySnapshot);

  if (!bridge.commands.includes("asher.ready")) return;
  try {
    const ready = await bridge.invoke<NativeReady>("asher.ready");
    if (ready.surface) {
      window.__ASHER_SURFACE__ = ready.surface;
    }
    if (ready.snapshot) {
      applySnapshot(ready.snapshot);
    }
  } catch (error) {
    console.error("failed to initialize asher shell bridge", error);
  }
}
...
function surfaceFromRuntime(snapshot?: ShellSnapshot): ShellSurface {
  if (window.__ASHER_SURFACE__) {
    return window.__ASHER_SURFACE__;
  }
  if (snapshot?.surface) {
