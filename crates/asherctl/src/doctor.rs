...
use asher_ipc::{IpcRequest, IpcResponse, ShellStatus, XwaylandStatus, send_request};
use serde::Serialize;
use std::{
    env, fs,
    path::{Path, PathBuf},
...
pub(crate) fn doctor_checks() -> Vec<DoctorCheck> {
    ...
    match ConfigPaths::discover() {
        Ok(paths) => {
            ...
            checks.push(check_binary("kestrel"));
            checks.push(check_binary("asher-shell"));
            checks.push(check_binary("asher-settings"));
...
fn check_binary(name: &str) -> DoctorCheck {
    if sibling_binary(name).is_some() || binary_in_path(name).is_some() {
        ...
        DoctorCheck::fail(
            format!("binary:{name}"),
            "not found beside asherctl or in PATH",
        )
    }
}

fn check_session_file() -> DoctorCheck {
    let installed = Path::new("/usr/share/wayland-sessions/asher.desktop");
    if installed.exists() || Path::new("data/sessions/asher.desktop").exists() {
...
fn check_ipc() -> DoctorCheck {
    match send_request(&IpcRequest::Status) {
        Ok(IpcResponse::Status(status)) => {
            let message = format!(
                "Kestrel reachable, workspace {}, shell {:?}",
        ...
        Err(error) => DoctorCheck::warning("ipc", format!("Kestrel is not reachable: {error}")),
    }
}
...
fn check_session_backend_dependencies() -> DoctorCheck {
    if binary_in_path("pkg-config").is_none() {
        return DoctorCheck::warning(
            "session-deps",
            "pkg-config is needed to build Kestrel with --features session-backend",
        );
    }
...
fn check_session_services() -> Vec<DoctorCheck> {
    ...
    if binary_in_path("dbus-run-session").is_some() {
        ...
        checks.push(DoctorCheck::warning(
            "dbus-session",
            "install dbus-run-session for a private Asher session bus",
        ));
    }
...
fn check_portal_config() -> DoctorCheck {
    match portal_config_paths().into_iter().find(|path| path.exists()) {
        ...
        None => DoctorCheck::warning(
            "portal-config",
            "install data/xdg-desktop-portal/asher-portals.conf into xdg-desktop-portal's data or config path",
        ),
    }
}
...
fn portal_config_paths() -> Vec<PathBuf> {
    vec![
        PathBuf::from("data/xdg-desktop-portal/asher-portals.conf"),
        PathBuf::from("/etc/xdg/xdg-desktop-portal/asher-portals.conf"),
        PathBuf::from("/usr/local/share/xdg-desktop-portal/asher-portals.conf"),
        PathBuf::from("/usr/share/xdg-desktop-portal/asher-portals.conf"),
    ]
}
