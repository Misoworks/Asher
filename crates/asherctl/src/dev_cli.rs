...
impl DevPlan {
    ...
    fn from_targets(targets: &[DevTarget], default: Self) -> Self {
        ...
        for target in targets {
            match target {
                ...
                DevTarget::Kestrel => plan.kestrel = true,
                DevTarget::All => {
                    plan.web = true;
                    plan.shell = true;
                    plan.config = true;
    ...
    fn any(self) -> bool {
        self.web || self.shell || self.config || self.kestrel
    }
}
...
fn run_apply(root: &Path, plan: DevPlan, release: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if plan.kestrel {
        build_kestrel(root, release)?;
        println!("Kestrel rebuilt. Restart the session or nested compositor to use that binary.");
    }
...
fn run_watch(
    ...
    println!("Watching Asher dev sources under {}", root.display());
    loop {
        thread::sleep(poll);
...
pub(super) fn build_shell(root: &Path, release: bool) -> Result<(), Box<dyn std::error::Error>> {
    build_rust_package(root, "asher-shell", release)?;
    build_rust_package(root, "asher-settings", release)
}

pub(super) fn build_kestrel(root: &Path, release: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    run_process("building kestrel session backend", &mut command)
}
...
fn request_live(label: &str, request: IpcRequest) -> Result<(), Box<dyn std::error::Error>> {
    match send_request(&request) {
        ...
        {
            eprintln!("{label} skipped: Kestrel IPC is not available ({error})");
            Ok(())
        }
...
impl DevSnapshot {
    fn scan(root: &Path, plan: DevPlan) -> io::Result<Self> {
        Ok(Self {
            ...
            kestrel: if plan.kestrel {
                scan_paths(root, &kestrel_paths())?
            } else {
                FileSet::new()
            },
        })
    ...
    fn diff(&self, next: &Self) -> DevPlan {
        ...
        DevPlan {
            ...
            kestrel: changed(&self.kestrel, &next.kestrel),
        }
    }
}
...
fn web_paths() -> Vec<PathBuf> {
    [
        "crates/asher-shell/web/src",
        "crates/asher-shell/web/index.html",
        "crates/asher-shell/web/index.ts",
        "crates/asher-shell/web/package.json",
        "crates/asher-shell/web/bun.lock",
        "crates/asher-shell/web/tsconfig.json",
        "crates/asher-shell/web/vite.config.ts",
    ]
    .into_iter()
    .map(PathBuf::from)
    .collect()
...
fn shell_paths() -> Vec<PathBuf> {
    [
        "crates/asher-shell/src",
        "crates/asher-shell/Cargo.toml",
        "crates/asher-settings/src",
        "crates/asher-settings/Cargo.toml",
        "crates/asher-config/src",
        "crates/asher-config/Cargo.toml",
        "crates/asher-ipc/src",
        "crates/asher-ipc/Cargo.toml",
        "crates/asher-layout/src",
        "crates/asher-layout/Cargo.toml",
    ]
    .into_iter()
    .map(PathBuf::from)
    .collect()
...
fn kestrel_paths() -> Vec<PathBuf> {
    [
        "crates/kestrel/src",
        "crates/kestrel/Cargo.toml",
        "crates/asher-session/src",
        "crates/asher-session/Cargo.toml",
        "crates/asher-config/src",
        "crates/asher-config/Cargo.toml",
        "crates/asher-ipc/src",
        "crates/asher-ipc/Cargo.toml",
...
fn workspace_root() -> io::Result<PathBuf> {
    if let Some(root) = env::var_os("ASHER_WORKSPACE").map(PathBuf::from)
    ...
    Err(io::Error::new(
        io::ErrorKind::NotFound,
        "could not find the Asher workspace root; run from the repo or set ASHER_WORKSPACE",
    ))
}

fn is_workspace_root(path: &Path) -> bool {
    path.join("Cargo.toml").is_file() && path.join("crates/asher-shell/web/package.json").is_file()
}
