...
use asher_config::{
    AsherConfig, ConfigPaths, ConfigSource, list_config_backups, load_config, save_config_to_path,
};
use asher_ipc::{IpcRequest, IpcResponse, ShellStatus, send_request};
use serde::Serialize;
use std::{
    env, fs, io,
    path::{Path, PathBuf},
...
const COMPONENTS: [&str; 3] = ["kestrel", "asher-shell", "asher-session"];
...
pub fn open_config(json: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !paths.config_file.exists() {
        save_config_to_path(&paths.config_file, &AsherConfig::default())?;
    }
    let opener = config_opener();
    let status = Command::new("sh")
        .arg("-lc")
        .arg("exec ${VISUAL:-${EDITOR:-xdg-open}} \"$1\"")
        .arg("asherctl-open-config")
...
pub fn print_recovery_status(json: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if json {
        ...
        println!("Kestrel log: {}", status.kestrel_log.display());
...
fn recovery_status() -> Result<RecoveryStatus, Box<dyn std::error::Error>> {
    ...
    let kestrel_log = paths.log_file("kestrel");
    let shell_log = paths.log_file("asher-shell");
    let recent_shell_crashes = count_log_matches(&kestrel_log, "asher shell exited")?
        + count_log_matches(&kestrel_log, "shell crash limit reached")?;
