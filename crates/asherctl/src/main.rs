...
use asher_config::{ConfigSource, load_config};
use asher_ipc::{IpcRequest, IpcResponse, ShellStatus, StatusPayload, send_request};
use asher_layout::{ProfileId, WindowId, WorkspaceId, mode_for_profile};
...
use output_cli::{OutputCommand, list_outputs, set_output_scale};
use recovery_cli::{list_recovery_backups, rollback_config};
use workspace_cli::{list_profiles, list_workspaces};

#[derive(Debug, Parser)]
#[command(name = "asherctl", about = "Control and inspect Asher")]
struct Cli {
    #[command(subcommand)]
    command: Command,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli.command {
        ...
        Command::Output(OutputCommand::List) => list_outputs(cli.json)?,
        Command::Output(OutputCommand::Scale { scale, output }) => set_output_scale(output, scale)?,
    }
...
fn print_status(json: bool) -> Result<(), Box<dyn std::error::Error>> {
    if let Ok(IpcResponse::Status(status)) = send_request(&IpcRequest::Status) {
        print_status_payload(json, status, "live Kestrel IPC".to_string())?;
        return Ok(());
    }
    ...
    let status = StatusPayload {
        ...
        xwayland: if loaded.config.compositor.xwayland {
            asher_ipc::XwaylandStatus::Unavailable
        } else {
            asher_ipc::XwaylandStatus::Disabled
        },
