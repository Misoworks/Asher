use asher_config::{AsherConfig, load_config};
use asher_ipc::{IpcRequest, IpcResponse, ProfileSummary, WorkspaceSummary, send_request};
use asher_layout::{ProfileId, WorkspaceId, mode_for_profile};
...
fn fallback_workspaces(config: &AsherConfig) -> Vec<WorkspaceSummary> {
...
fn fallback_profiles(config: &AsherConfig) -> Vec<ProfileSummary> {
