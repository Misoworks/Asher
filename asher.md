# Asher Desktop Environment Specification

## 0. Purpose

Asher is a standalone Linux desktop environment built around a custom Wayland compositor and a modular shell.
...
Asher must be useful as a desktop environment by itself. Apps such as file manager, terminal, app store, browser, notes, email, etc. are out of scope and will be built separately or provided by the host distribution.
...
- **Asher**: the desktop environment.
- **Kestrel**: the Wayland compositor.
- **Asher Shell**: the visible desktop shell UI.
- **Glass**: dynamic glass/blur material.
- **Glass Blur**: softer mica-like material.
...
Asher is not just a pretty shell.

Asher is a desktop environment where each workspace can have a different workflow model.
...
2. Use a custom compositor called Kestrel.
...
10. Provide internal material primitives for Asher Shell surfaces.
11. Support per-workspace Shell Profiles.
12. Support multiple workflow modes.
13. Provide a CLI called `asherctl`.
...
Asher may provide integration surfaces for these later, but it must not require them.
...
Kestrel should be built with Smithay.

Reasons:

- Rust-native Wayland compositor framework.
- Good fit for custom compositor behavior.
- Allows Asher to define its own shell, modes, materials, and layout model.
...
Asher should be split into clear components.
...
### 6.3 Kestrel Compositor Responsibilities
...
Kestrel must remain stable even if the shell crashes.

### 6.4 Asher Shell Responsibilities
...
- Explicitly designed for Asher Shell.
...
This runs Kestrel inside an existing Wayland session.
...
Kestrel must support these protocol groups.
...
output-management
foreign-toplevel-management or Asher equivalent
```
...
Kestrel must implement support for the protocol where possible.
...
Asher Shell should not need to use the public protocol path for every internal surface.

Kestrel should expose an internal material API for trusted shell surfaces.
...
Kestrel must provide compositor-level blur.
...
Blur can be expensive. Kestrel must include a performance-conscious render path.
...
Kestrel should avoid hardcoding one window management model. The layout engine and active mode should decide behavior.

### 7.9 Shell Surface Layers

Kestrel must support shell surfaces with layers:
...
Kestrel must treat these as security-sensitive:
...
Asher Shell is the visible desktop interface.
...
- Autohide.
- Intellihide.
- Glass/Glass Blur/solid material.
- Icon zoom optional.
- Compact mode.
- Touch-friendly mode.
...
Asher should implement notification UI compatible with Linux desktop notification systems.
...
- Overview.
- Smooth animations.
- Glass/Glass Blur material by default.
...
```txt
/usr/share/asher/default-config/
```
...
`asherctl` is the primary configuration and debugging interface.

It allows users, scripts, future settings apps, and Codex/AI tools to interact with Asher safely.
...
asherctl doctor
asherctl reload
asherctl safe-mode enable
asherctl safe-mode disable
asherctl profiles list
asherctl profiles show <id>
asherctl profiles validate <path-or-id>
asherctl profiles set-workspace <workspace> <profile>
asherctl modes list
asherctl workspaces list
asherctl workspaces switch <workspace>
asherctl workspaces set-profile <workspace> <profile>
asherctl effects status
asherctl effects blur enable
asherctl effects blur disable
asherctl materials list
asherctl config path
asherctl config validate
asherctl config open
asherctl recovery status
asherctl recovery rollback
asherctl debug overlay enable
asherctl debug overlay disable
```

### 13.3 Future Commands

```sh
asherctl patches list
asherctl patches apply <patch>
asherctl patches revert <patch>
asherctl extensions list
asherctl extensions enable <id>
asherctl extensions disable <id>
asherctl ai propose "<instruction>"
asherctl ai explain <patch>
```
...
```sh
asherctl profiles list --json
```

This allows a future settings app to use `asherctl` or the underlying IPC as a backend.
...
Asher should look good out of the box.
...
Asher should be able to derive colors from the current wallpaper.
...
Asher should use crisp, quick, precise animations.
...
Browser Mode is the most distinctive Asher workflow.
...
Not every app supports perfect restore. Asher should do best effort.
...
- Window previews.
- Workspace switching.
- Glass/Glass Blur material.
- Familiar behavior.

### 17.3 Default Use

Dock Mode should be a strong candidate for the default Asher experience.
...
```txt
~/.config/asher/profiles/
~/.config/asher/materials/
~/.config/asher/keybindings.toml
~/.config/asher/rules.toml
~/.config/asher/extensions/
```
...
Extensions allow users to customize Asher without modifying core code.
...
```sh
WAYLAND_DISPLAY=asher-1 alacritty
WAYLAND_DISPLAY=asher-1 weston-terminal
WAYLAND_DISPLAY=asher-1 foot
WAYLAND_DISPLAY=asher-1 gtk4-demo
```
...
```txt
/usr/share/wayland-sessions/asher.desktop
```
...
Kestrel should include debug overlay showing:
...
Asher must not assume Glacier OS.
...
Asher should eventually provide or integrate with XDG Desktop Portals.
...
Asher must not ignore accessibility.
...
The terminal/file commands should be configurable because Asher does not ship its own apps.
...
Asher should integrate with existing Linux services instead of reinventing everything.
...
Because Asher does not ship apps, users must configure:
...
Asher should eventually include a secure lock screen.
...
### Milestone 2: Nested Kestrel Prototype
...
opens a visible terminal in nested Asher.
...
Done when shell can be killed/restarted without killing Kestrel.

### Milestone 5: Config and asherctl
...
- `asherctl profiles list`.
- `asherctl workspaces list`.
- `asherctl workspaces set-profile`.
- JSON output.
...
Done when Asher can run from a TTY/display manager and launch Wayland/XWayland apps.
...
```txt
Create the initial Asher Rust workspace and implement a Kestrel nested Wayland compositor prototype using Smithay.

Requirements:
- Create a Cargo workspace with crates for kestrel, asher-layout, asher-config, asher-ipc, asher-shell, asher-session, and asherctl.
- Implement only enough functionality for the first nested compositor prototype.
- Kestrel must run with `cargo run -p kestrel -- --nested`.
- Kestrel must create a Wayland socket and print the `WAYLAND_DISPLAY` value to use.
- Kestrel must accept xdg-shell toplevel clients.
- Kestrel must render a basic background.
- Kestrel must map/unmap client windows.
- Kestrel must support basic pointer and keyboard focus.
- Kestrel must support moving windows with a modifier-drag or simple hardcoded key/mouse behavior.
- Add structured modules for backend, input, output, protocols, render, window, and recovery.
- Add logging with useful debug output.
- Add a placeholder config loader that reads `~/.config/asher/config.toml` if present and falls back to defaults.
- Add `asherctl status` as a placeholder command that can later connect over IPC.
- Add a README with build/run/test instructions.
- Do not implement the full shell yet.
- Do not implement AI yet.
- Do not implement the distro.
- Keep architecture compatible with the full asher.md spec.
```
...
Asher is usable as an experimental DE when:
...
Asher is actually Asher when:

- Kestrel has compositor blur.
- Glass/Glass Blur materials work.
...
1. **The compositor must be stable.**
   The shell can be expressive; Kestrel must be boring where stability matters.
...
8. **Do not build the distro first.**
   Asher must be usable on existing Linux systems.

9. **Do not require custom apps.**
   Asher is the DE. Apps are separate.

10. **The visual identity matters.**
    Asher should not look like stock GNOME, old KDE, or random Hyprland rice.

---

## 35. Final Target

The final Asher experience should feel like this:
